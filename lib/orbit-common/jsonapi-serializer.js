      ...
      if (relKeys.length > 0) {
        ...
        relKeys.forEach(function(key) {
          ...
          if(!serialized[key]){
            ...
            var idsArray = [];
            serialized[key].forEach(function(el){
              idsArray.push(_this.schema.localToRemoteId(relatedModel, el.get("clientid")));
            });
            serialized[key]= idsArray;
          }else {
            serialized[key]= _this.schema.localToRemoteId(relatedModel, serialized[key].get("clientid"));
          }
        });
      }
