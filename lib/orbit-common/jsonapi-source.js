...
var JSONAPISource = Source.extend({
  ...
  init: function(schema, options) {
    ...
    // If `SerializerClass` is obtained through the _super chain, dereference its
    // wrapped function, which will be the constructor.
    //
    // Note: This is only necessary when retrieving a *constructor* from a class
    // hierarchy. Otherwise, `superWrapper` "just works".
  ...
  _transform: function(operation) {
    ...
    throw new OperationNotAllowed('JSONAPISource#transform could not process operation: ' + operation.op + ' with path: ' + operation.path.join('/'));
  },
  ...
  _findLink: function(type, id, link) {
    var _this = this;
    return this.ajax(this.resourceLinkURL(type, id, link), 'GET')
      .then(function(raw) {
        var linkDef = _this
          .schema
          .linkDefinition(type, link);
        var relId = _this
          .serializer
          .deserializeLink(linkDef.model, raw);
  ...
  //
  //  _findLinked: function(type, id, link, relId) {    if (relId === undefined) {
  //      id = this.getId(type, id);
  //
  //      var record = this.retrieve([type, id]);      if (record) {        relId
  // = record.__rel[link];        if (record.__meta.links &&
  // record.__meta.links[link]) {          var linkMeta =
  // record.__meta.links[link];
  //
  ...
  _transformAdd: function(operation) {
    ...
    var json = this
      .serializer
      .serialize(type, operation.value);
  ...
  _transformReplace: function(operation) {
    ...
    return this.ajax(this.resourceURL(type, id), 'PUT', {data: json})
      .then(function(raw) {
  ...
  _transformAddLink: function(operation) {
    ...
    var relResourceType = this
      .serializer
      .resourceType(relType);
    var relResourceId = this
      .serializer
      .resourceId(relType, relId);

    var method = 'POST';
    var json = {};
    json[relResourceType] = relResourceId;

    return this.ajax(this.resourceLinkURL(type, id, link), method, {data: json})
      .then(function() {
        _this._transformCache(operation);
      });
  },
  ...
  _transformRemoveLink: function(operation) {
    ...
    return this.ajax(this.resourceLinkURL(type, id, link, relId), 'DELETE')
      .then(function() {
        _this._transformCache(operation);
      });
  },
  ...
  _transformUpdateAttribute: function(operation) {
    ...
    this
      .serializer
      .serializeAttribute(type, record, attr, serialized);
  ...
  _addRecordToCache: function(type, record, parentOperation) {
    var operation = {
      op: 'add',
      path: [
        type, this.getId(type, record)
      ],
      value: record
    };

    if (parentOperation)
      operation = parentOperation.spawn(operation);
  ...
  _findMany: function(type, ids) {
    var _this = this;
    return this.ajax(this.resourceURL(type, ids), 'GET')
      .then(function(raw) {
  ...
  _findQuery: function(type, query) {
    ...
    return this.ajax(this.resourceURL(type), 'GET', {data: query})
      .then(function(raw) {
        var records = _this.deserialize(type, null, raw);
        return _this
          .settleTransforms()
  ...
  _transformCache: function(operation) {
    ...
    if (operation.op === 'add') {
      pathToVerify = operation
        .path
        .slice(0, operation.path.length - 1);
    } else {
      pathToVerify = operation.path;
    }

    if (this.retrieve(pathToVerify) !== undefined) {
      // transforming the cache will trigger a call to `_cacheDidTransform`, which
      // will then trigger `didTransform`
      this
        ._cache
        .transform(operation);

    } else if (operation.op === 'replace') {
      // try adding instead of replacing if the cache does not yet contain the data
      operation.op = 'add';
      this._transformCache(operation);
      ...
      //
      // NOTE: this is not an error condition, since the local cache will often be
      // sparsely populated compared with the remote store
  ...
  ajax: function(url, method, hash) {
    ...
    return new Orbit.Promise(function(resolve, reject) {
      ...
      hash.success = function(json) {
        ...
        localforage.getItem(projectCode + '-offlineMode').then(function(offline) {
          if (offline === null) {
            if (hash.url === "/api/projects") {
              var found = false;
              json
                .projects
                .forEach((project) => {
                  if (project.projectCode === projectCode) {
                    found = true;
                    if (!project.infos.offlineMode) {
                      localstorage.setItem(projectCode + '-offlineMode', 'false');
                      localforage.setItem(projectCode + '-offlineMode', 'false').then(function() {
                        _this.tmpDataStore = [];
                        resolve(json);
                      });
                    } else if (project.infos.offlineMode === 'false') {
                      localstorage.setItem(projectCode + '-offlineMode', 'false');
                      localforage.setItem(projectCode + '-offlineMode', 'false').then(function() {
                        _this.tmpDataStore = [];
                        resolve(json);
                      ...
                      localforage.setItem(projectCode + '-offlineMode', 'true').then(function() {
                        localstorage.setItem(projectCode + '-offlineMode', 'true');
                        localforage.setItem(url, JSON.stringify(json)).then(function() {
                            for (var i = 0; i < _this.tmpDataStore.length; i++) {
                              localforage.setItem(_this.tmpDataStore[i].name, _this.tmpDataStore[i].data);
                            }
                            resolve(json);
                        });
                      });
              ...
              _this.tmpDataStore.push({name: url, data: JSON.stringify(json)});
              resolve(json);
            }
          } else {
            if(offline === 'false') {
      ...
      hash.error = function(jqXHR, textStatus, errorThrown) {
        ...
        localforage.getItem(projectCode + '-offlineMode').then(function(offline) {
          ...
          if (offline === null) {
            ...
            if (!window.navigator.onLine && offline === 'true') {
              console.log("offline mode is working")
  ...
  deserialize: function(type, id, data, parentOperation) {
    ...
    // transformation.
    //
    // Note: this parent operation is not actually performed on this source. It is
    // only created to establish a common ancestor.
    if (!parentOperation) {
